...
async function Spellstrike()
{
  ...
  const map_weap = weapons.map(p => p.name);
      ...
      let flavor = `<strong>Spellstrike</strong><br><a class="entity-link content-link" data-pack="pf2e.spells-srd" data-id="${spc.cId}"><strong>${spc.data.item.name}</strong></a> (${dos})<div class="tags">${ttags}</div><hr>`;
