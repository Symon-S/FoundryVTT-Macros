...
async function Spellstrike()
{
  ...
  for (let token of canvas.tokens.controlled) {
    ...
    let entries = token.actor.itemTypes.spellcastingEntry;
    ...
    entries.forEach( e => {
      if (e.isRitual) { return; }
        ...
        if (standby && sp.level < token.actor.itemTypes.spell.find(s => s.data.flags.pf2e.standbySpell === true).baseLevel) { return; }
