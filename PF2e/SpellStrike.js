...
async function Spellstrike()
{
      ...
      token.actor.itemTypes.spellcastingEntry.forEach( e => {
        if (e.isRitual) { return; }
          ...
          if(!e.isPrepared && !e.isFlexible && !e.isFocusPool && !sp.isCantrip && sp.uses.value < 1) { return; }
            ...
            if (spa.spell.data.data.spellType.value === 'utility' || spa.spell.data.data.spellType.value === 'heal') { 
  ...
  const map_weap = weapons.map(p => p.name);
      ...
      let flavor = `<strong>Spellstrike</strong><br><a class="entity-link content-link" data-pack="pf2e.spells-srd" data-id="${spc.cId}"><strong>${spc.data.item.name} cast at LV${spc.lvl}</strong></a> (${dos})<div class="tags">${ttags}</div><hr>`;
      if (spc.slug === 'acid-splash') { flavor = `<strong>Spellstrike</strong><br><a class="entity-link content-link" data-pack="pf2e.spells-srd" data-id="${spc.cId}"><strong>${spc.data.item.name}</strong></a> (${dos})<div class="tags">${ttags}</div>`; }
      if (spc.isSave) {
