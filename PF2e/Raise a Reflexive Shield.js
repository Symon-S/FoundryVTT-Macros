...
for await (const actor of actors) {
    ...
    if (existing) {
        await existing.delete();
                ...
                if (token.actor.itemTypes.armor.filter(s => s.isEquipped === true && s.data.data.armorType.value === "shield" && s.data.data.hp.value > s.data.data.brokenThreshold.value).length != 0) {
