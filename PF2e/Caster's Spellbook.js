...
You can keep the dialog open and minimize it, or move it to a more convenient location
to keep using it until you either refresh or want to close it.
A token must be selected to use the macro.
*/
...
const script = async function Spells(id){
  ...
  async function Spell() {
    ...
    spells.forEach((value,index) => {
      ...
      async function Cast() { value.spell.spell.toMessage(); spells = value; }
...
<div><strong>Choose a Spellcasting Entry:</strong></div><script>${script}</script>`;
...
await Diag1({title: "Caster's Spellbook", content});

async function Diag1({title,content} = {}) {
	await new Promise(() => {
		new Dialog({
			title,
                        content,
